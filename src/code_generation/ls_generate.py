...
class LongStringGenerator:
    ...
    #     system_message = """You are a code generation AI model for a robot called Pepper that has to check if the code generated by another model is correct, and if not, correct it."""

    #     text_prompt = f"""
    #     Your main task is to fix the code given, to fix this code make sure that it follows the following rules:

    #     - MANDATORY: Just return the code with the next format:
    #     - Your output needs to be formatted in markdown as a python code snippet which looks like ```python <CODE>```
    #     - If the code is correct, just return the code as it is

    #     - Assume there is a self.tm object that is already instantiated, so using this will not generate an error

    #     - For functions that require a place name, use the exact syntax from the list: {self.place_names}, if the exact syntax is not used, replace it with the most similar one from the list. Use the syntax from the list when calling the codebase functions.
    #     - For example if the code has a `self.tm.go_to_place("Dinning")` you should replace it with `self.tm.go_to_place("dinning_room")`

    #     - For functions that require an object name, use the exact syntax from the list: {self.objects}, if the exact syntax is not used, replace it with the most similar one from the list. Use the syntax from the list when calling the codebase functions.
    #     - For example if the code has a `self.tm.find_object("water")` you should replace it with `self.tm.find_object("bottle")`

    #     - If the `self.tm.q_a_speech(<tag>)` function is used, make sure that the tag is from the list: {self.question_tags}, if the tag is not from the list, replace it with the most similar one from the list. Use the syntax from the list when calling the codebase functions.
    #     - If you need to ask a question that is not listed just use the `talk` method to say the question and the `speech2text_srv` followed to save the answer. Use the syntax from the list when calling the codebase functions.

    #     - The allowed functions are listed above, make sure they use correct syntax, if not please fix it
    #     - For example if the code has a `self.tm.speak("Hello world")` you should replace it with `self.tm.talk("Hello world")`, because the self.tm.speak*() function does not exist.

    #     # Code to check:
    #     {code}

    #     # self.tm allowed functions:
