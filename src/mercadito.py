...
class MERCADITO(object):
    def __init__(self):
        ...
        self.is_done = False
        states = ['INIT', 'FOLLOW_YOU','FINISH','MERCADITO_DONE']
    ...
    def on_enter_FOLLLOW_YOU(self):
        print(self.consoleFormatter.format("FOLLOR_YOU", "HEADER"))
        self.tm.follow_you()
        stop_thread = threading.Thread(target=self.stop_thread)
        stop_thread.start()
        hi_pepper_thread = threading.Thread(target=self.hi_pepper_thread)
        hi_pepper_thread.start()
