...
from speech_utilities_msgs.srv import hot_word_srv 
...
class CARRY_MY_LUGGAGE(object):
    def __init__(self) -> None:
        ...
        self.STATES = ['INIT', 'LOOK_FOR_BAG', "MOVE_TO_BAG", "GRAB_BAG", "FOLLOW_YOU", "GO_BACK", "END"]
        self.TRANSITIONS = [{'trigger': 'zero', 'source': 'CARRY_MY_LUGGAGE', 'dest': 'INIT'},
        ...
        self.follow_you_srv = rospy.ServiceProxy("/navigation_utilities/follow_you_srv", follow_you_srv)

        self.getLabelsSubscriber = rospy.Subscriber('/perception_utilities/get_labels_publisher', get_labels_msg, self.callback_get_labels_subscriber)
    ...
    def callback_get_labels_subscriber(self, msg):
        ...
        heights = msg.heights
        ids = msg.ids
        closest_person = {'id':None, 'width':None, 'height':None, 'x':None, 'y':None, 'status':None}
        for label in range(len(labels_msg)):
            if labels_msg[label] == "person" and (closest_person['id'] is None or closest_person['width'] < widths[label]):
